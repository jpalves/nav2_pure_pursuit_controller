 ...
 */
template<typename Iter, typename Getter> Iter min_by(Iter begin, Iter end, Getter getCompareVal){
	if (begin == end) {
    		return end;
  	}
...
void PurePursuitController::setSpeedLimit(const double & speed_limit, const bool & percentage){
	...
	if (desired_linear_vel_ > speed_limit && percentage) {
...
geometry_msgs::msg::TwistStamped PurePursuitController::computeVelocityCommands(
  ...
  nav2_core::GoalChecker * goal_checker __attribute__((unused))){
